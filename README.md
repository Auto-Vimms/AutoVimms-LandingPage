...
🌐 **Live site:** [auto-vimms-landing-page.vercel.app](https://auto-vimms-landing-page.vercel.app)
...
│   ├── components.css     # buttons, cards, form, pills
│   └── utilities.css      # animations, reveal, responsive
└── src/
    ├── main.js             # entry point — initializes everything
    ├── config/
    │   └── config.js              # WhatsApp, email, Instagram, submit method
    ├── domain/
    │   └── budget/
    │       ├── BudgetForm.js      # collects and validates form data
    │       └── BudgetService.js   # decides which delivery method to use
    ├── infrastructure/
    │   └── delivery/
    │       ├── WhatsAppDelivery.js   # sends request via WhatsApp
    │       └── EmailDelivery.js      # placeholder for future email integration
    └── ui/
        ├── Toast.js            # confirmation notification
        ├── RevealObserver.js   # scroll reveal animation
        └── ContactLinks.js     # injects WhatsApp/Instagram/Email links from CONFIG
```
...
> ⚠️ The project uses ES Modules (`<script type="module">`), so it must be served by a local server (like Live Server) — opening `index.html` directly via `file://` will not work.

---

## ⚙️ Updating Contact Info

WhatsApp number, email, and Instagram handle are centralized in:

```bash
src/config/config.js
```

Changing the values there automatically updates the contact section, the footer links, and the WhatsApp message generated by the budget form.

---

## 🚢 Deployment

The project is deployed automatically via **Vercel**. Every push to the `main` branch triggers a new production deployment.
